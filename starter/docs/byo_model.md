...
**Constraint reminder:** hosted endpoints are fine for *development*, but your submitted run must use open-weight models totalling ≤ 96 GB of reported VRAM, on an endpoint that names the exact checkpoint — see the [competition page](https://www.kaggle.com/competitions/OpenAgent-Coding/overview). Check any candidate with `python scripts/estimate_vram.py <repo-id>`. Closed-weight models (GPT, Claude, Gemini) are out of scope everywhere. Bedrock's fully-managed `qwen3-coder-30b-a3b` is eligible (it names its checkpoint); Bedrock Custom Model Import is not viable for a hackathon team (Provisioned-Throughput-only, $21–50/hr). Good defaults: `Qwen/Qwen3.8-27B-FP8` (~30 GB) if you're self-hosting, `Qwen/Qwen3.6-27B-FP8` (37 GB) via the [provided UW–Madison endpoint](uw_madison_endpoint.md), or `Qwen/Qwen2.5-Coder-32B-Instruct-AWQ` (28 GB) — the most widely hosted, and the easiest no-GPU path.
