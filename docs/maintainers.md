...
- **Hackathon format confirmed after revisiting the leaderboard question (July 2026, Chris).** The scored-prediction-competition alternative was reconsidered and rejected on two grounds. (1) *Integrity is a wash at our scale:* with a test set of tens of pages, a hidden set is defeated by simply hand-transcribing the public test images, so a "real" leaderboard rests on the same honor-system + top-10 re-run backstop as self-reporting; padding the hidden set with held-out Bentham/Escher pages (GT free, hand-labeling impractical at ~400 pages) would fix that but tilts the headline metric toward public data that's plausibly in VLM training sets. (2) *GT scarcity decides it:* Scott realistically has time for a few dozen GT pages this year, and a hidden/calibration split makes both halves too thin to mean anything — the hackathon releases every scarce GT page to every team. What the leaderboard would have bought (auto-scoring removes honest self-report errors; engagement loop; no standings-post upkeep) is real but not worth the split. Revisit only if the GT pool grows ~100+ pages (the prediction-format files are one commit back in this branch's history, 27ff5da–f658fa0). The same fence exists for Efficient Coder (claims-CSV leaderboard, artifacts in that repo's main history) — that decision lives in the Efficient Coder repo, not here.
