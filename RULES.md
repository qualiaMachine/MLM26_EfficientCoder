...
- **Open weights, within a 96 GB budget.** Your submitted run may use any open-weight model(s) whose total reported VRAM is ≤ 96 GB, served on hardware totalling ≤ 96 GB, with weights quantized no lower than 4-bit — see [Model rules](https://github.com/qualiaMachine/MLM26_EfficientCoder/blob/main/README.md#model-rules). Closed-weight models (GPT, Claude, Gemini) are out of scope anywhere in your system, including "just the planner." Development and prototyping on any model is fine — the rule binds the submitted run.
